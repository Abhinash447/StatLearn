...
export const PersonalizedLearningPath = () => {
  const { learningPath, user, setActiveTab, enrollInIGOT, t } = useApp();
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-2xl p-6 sm:p-8 text-white shadow-xl border border-indigo-700/60 relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="bg-indigo-950/80 p-4 rounded-2xl border border-indigo-700/60 text-center flex-shrink-0">
            <p className="text-xs text-slate-400">{t("roadmapCompletion") || "Roadmap Completion"}</p>
            <p className="text-3xl font-black text-amber-400 mt-0.5">{progressPercentage}%</p>
            <p className="text-[11px] text-slate-300 mt-1">
              {completedStepsCount} of {learningPath.length} {t("milestones") || "Milestones"}
            </p>
          </div>
        </div>
      </div>
        ...
        {learningPath.map((step) => {
          ...
          return (
            ...
            >
              {/* Stepper Node Icon */}
              <div
                className={`absolute left-0 sm:left-2 top-5 w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center font-bold text-sm shadow-md transition-all z-10 ${
                  isCompleted
                    ? "bg-emerald-600 text-white shadow-emerald-500/30"
                    : isInProgress
                    ? "bg-gradient-to-tr from-amber-500 to-orange-500 text-slate-950 shadow-amber-500/30 ring-4 ring-amber-100"
                    : "bg-white text-slate-400 border-2 border-slate-300"
                }`}
              >
                {isCompleted ? <CheckCircle2 className="w-6 h-6" /> : <span>{step.stepNumber}</span>}
              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-3 bg-blue-50/70 p-3 rounded-xl border border-blue-100 text-xs text-blue-950 flex items-start gap-2">
                      ...
                      <div>
                        <span className="font-bold text-blue-900">{t("whyThisStep") || "Why this step?"} </span>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 flex-shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                    <div className="text-left sm:text-right text-xs">
                      <p className="text-slate-400 text-[10px]">{t("ecosystemProvider") || "Ecosystem Provider"}</p>
                    ...
                    {isCompleted ? (
                      <span className="px-3.5 py-1.5 rounded-xl bg-emerald-100 text-emerald-800 font-bold text-xs flex items-center gap-1">
                        <CheckCircle2 className="w-3.5 h-3.5" /> {t("benchmarkMet") || "Completed"}
                      </span>
                    ) : isInProgress ? (
                      <button
                        onClick={() => {
                      ...
                      >
                        ...
                        <span>{t("continueBtn") || "Continue Learning"}</span>
                      </button>
                    ) : (
                      <button
                        onClick={() => {
                      ...
                      >
                        <span>{t("startMilestoneBtn") || "Start Milestone"}</span>
