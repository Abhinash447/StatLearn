...
export const AiAssistant = () => {
  const { user, competencies, skillGaps, language, setLanguage, setActiveTab } = useApp();
...
I have loaded your complete competency dossier (${user.designation}, ${user.department}). How can I assist your statistical learning journey today?`,
  ...
  const starterQuestions = [
    ...
    "Why was Python recommended?",
    "Why is AI/ML a skill gap?",
    "Explain stratified sampling.",
    "Recommend iGOT courses for me.",
    "What NSSTA training should I attend?",
  ...
  return <div className="space-y-6 max-w-5xl mx-auto flex flex-col h-[calc(100vh-140px)]">
      {
    /* Header Banner */
  }
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-extrabold text-base sm:text-lg text-white">
                StatSkill AI Virtual Assistant
              </h1>
            ...
            <p className="text-[11px] text-slate-300">
              Personalized for {user.name} ({user.designation}, {user.department})
            </p>
          </div>
        </div>
    ...
    /* Language selector in Chat */
        ...
        <div className="flex items-center bg-slate-800 border border-slate-700 rounded-xl p-1 text-xs">
          ...
          {["en", "hi", "te"].map((lang) => <button
    key={lang}
    onClick={() => setLanguage(lang)}
    className={`px-2.5 py-1 rounded-lg font-bold transition-all ${language === lang ? "bg-amber-400 text-slate-950 shadow-sm" : "text-slate-300 hover:text-white"}`}
  >
    ...
    >
                {isBot && <div className="w-8 h-8 rounded-xl bg-blue-600 text-white flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-sm mt-0.5">
                    <Bot className="w-4 h-4" />
                  </div>}

                <div
      className={`max-w-[85%] sm:max-w-[75%] p-4 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-sm ${isBot ? "bg-slate-50 border border-slate-200 text-slate-800 whitespace-pre-wrap" : "bg-blue-600 text-white font-medium rounded-tr-none"}`}
    >
                  <p>{msg.text}</p>
                  <span
      className={`block text-[10px] mt-2 font-mono ${isBot ? "text-slate-400" : "text-blue-200 text-right"}`}
    >
                    {msg.timestamp}
                  </span>
                </div>

                {!isBot && <div className="w-8 h-8 rounded-xl bg-amber-500 text-slate-950 flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-sm mt-0.5">
  ...
  >
          <input
    type="text"
    placeholder={language === "hi" ? "\u0905\u092A\u0928\u0947 \u0915\u094C\u0936\u0932, iGOT \u0915\u094B\u0930\u094D\u0938 \u092F\u093E \u0938\u093E\u0902\u0916\u094D\u092F\u093F\u0915\u0940\u092F \u092A\u094D\u0930\u0936\u094D\u0928 \u092A\u0942\u091B\u0947\u0902..." : language === "te" ? "\u0C2E\u0C40 \u0C28\u0C48\u0C2A\u0C41\u0C23\u0C4D\u0C2F\u0C3E\u0C32\u0C41 \u0C32\u0C47\u0C26\u0C3E iGOT \u0C15\u0C4B\u0C30\u0C4D\u0C38\u0C41\u0C32 \u0C17\u0C41\u0C30\u0C3F\u0C02\u0C1A\u0C3F \u0C05\u0C21\u0C17\u0C02\u0C21\u0C3F..." : "Ask about your skill gaps, recommended courses, or statistical concepts..."}
    value={inputQuery}
