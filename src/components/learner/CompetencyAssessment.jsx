...
export const CompetencyAssessment = () => {
  const { finishQuizSession, setActiveTab, updateCompetencyScore, showToast, t } = useApp();
  ...
  const handleSubmitAssessment = () => {
    ...
    Object.entries(compScoresMap).forEach(([compName, val]) => {
      ...
      if (compPct >= 70) {
        ...
        impacts.push({ name: compName, score: 85, isAbsolute: true });
      } else if (compPct >= 40) {
        weaknesses.push(compName);
        impacts.push({ name: compName, score: 55, isAbsolute: true });
      } else {
        weaknesses.push(compName);
        impacts.push({ name: compName, score: 30, isAbsolute: true });
      }
    });
  ...
  const handleApplyToProfile = () => {
    ...
    showToast(t("calibrationSuccess"), "success");
      ...
      <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-300">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm text-center">
          ...
          <h1 className="text-3xl font-black text-slate-900 mt-1">
            Diagnostic Score: <span className={resultsData.percentage >= 60 ? "text-emerald-600" : "text-amber-600"}>{resultsData.percentage}%</span>
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            {resultsData.score} of {resultsData.total} Statistical Questions Answered Correctly
          </p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            {Object.entries(resultsData.categoryScores).map(([cat, val]) => (
              <div key={cat} className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                <p className="text-[11px] font-bold text-slate-600 capitalize">{cat}</p>
                <p className="text-lg font-black text-slate-900 mt-0.5">{val.pct}%</p>
                <p className="text-[10px] text-slate-400">{val.correct}/{val.total} Correct</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-emerald-200 p-6 shadow-sm">
            ...
            <ul className="space-y-2 text-xs">
              {resultsData.strengths.map((s, i) => (
                <li key={i} className="flex items-center gap-2 text-slate-700 bg-emerald-50/50 p-2.5 rounded-lg border border-emerald-100">
          ...
          <div className="bg-white rounded-2xl border border-rose-200 p-6 shadow-sm">
            ...
            <ul className="space-y-2 text-xs">
              {resultsData.weaknesses.map((w, i) => (
                <li key={i} className="flex items-center gap-2 text-slate-700 bg-rose-50/50 p-2.5 rounded-lg border border-rose-100">
        ...
        <div className="bg-gradient-to-r from-blue-900 to-indigo-950 rounded-2xl p-6 text-white shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h3 className="font-bold text-base">{t("readyToUpgradeTitle")}</h3>
          ...
          >
            ...
            <span>{t("generateLearningPathBtn")}</span>
          </button>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500">{t("diagnosticCadre")}</span>
          </div>
          <h1 className="text-xl font-extrabold text-slate-900 mt-1">
            {t("diagnosticAssessmentTitle")}
          </h1>
        </div>

        <div className="flex items-center gap-4 text-xs font-semibold">
          <span className="text-slate-500 flex items-center gap-1">
            <Clock className="w-4 h-4 text-slate-400" /> {t("untimedSelfPaced")}
          </span>
          <span className="bg-slate-100 text-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 font-mono">
            {t("answeredCountLabel")}: {answeredCount}/{totalQuestions}
          </span>
        </div>
      </div>
