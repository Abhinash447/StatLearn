...
export const NsstaTraining = () => {
  const { nsstaProgrammes, nominateInNSSTA, user, setActiveTab } = useApp();
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <span className="bg-amber-500/20 text-amber-300 text-xs font-semibold px-3 py-1 rounded-full border border-amber-400/30">
              NSSTA Training Programme Advisory Committee (TPAC) 2026-27
            </span>
            <h1 className="text-2xl sm:text-3xl font-extrabold mt-2 tracking-tight">
              National Statistical Systems Training Academy (NSSTA)
            </h1>
            <p className="text-slate-300 text-xs sm:text-sm mt-1 max-w-2xl leading-relaxed">
              Premier apex training institute for India's Official Statistical System located in Greater Noida, UP.
              Programmes are dynamically prioritized based on your critical capability diagnostics.
            </p>
          </div>
          ...
          <div className="bg-slate-900/80 p-4 rounded-2xl border border-slate-700 text-center flex-shrink-0">
            ...
            <p className="text-2xl font-black text-amber-400 mt-0.5">{user.designation}</p>
            <p className="text-[11px] text-slate-300 mt-1">NSS & Central Cadre Eligible</p>
          </div>
        </div>
      </div>
      ...
      /* AI Rationale */
                  ...
                  <div className="bg-indigo-50/70 p-3.5 rounded-xl border border-indigo-100 text-xs text-indigo-950 leading-relaxed">
                    <p className="font-bold text-indigo-900 flex items-center gap-1.5 mb-1">
                      ...
                      Official Training Recommendation Rationale:
                    </p>
      ...
      /* Metadata grid */
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600 pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span><strong>Start Date:</strong> {prog.startDate}</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span><strong>Faculty:</strong> {prog.faculty}</span>
                    </div>
                  </div>
                </div>
      ...
      /* Right Action */
                ...
                <div className="flex lg:flex-col items-center lg:items-end justify-between lg:justify-start gap-4 flex-shrink-0 pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                  <div className="text-left lg:text-right text-xs">
                    <p className="text-slate-400 text-[10px]">Cohort Availability</p>
                    <p className="text-lg font-black text-slate-800">{prog.seatsAvailable} Seats</p>
                  </div>

                  {isNominated ? <div className="px-4 py-2 rounded-xl bg-emerald-100 text-emerald-800 font-bold text-xs flex items-center gap-1.5 border border-emerald-300">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                      <span>Nomination Dispatched</span>
                    </div> : <button
