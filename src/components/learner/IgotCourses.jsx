...
export const IgotCourses = () => {
  const { igotCourses, enrollInIGOT, setActiveTab, t } = useApp();
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-800 text-xs font-bold border border-emerald-200">
              ...
              {t("igotConnected") || "🟢 Connected — Demo Integration"}
            </span>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-200 text-right">
            <p className="text-[10px] text-slate-500 uppercase font-semibold">
              {t("kpiCoursesCompleted") || "Enrolled Courses"}
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-col sm:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder={t("searchPlaceholderLearner") || "Search iGOT courses by title, skill, or department..."}
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <option value="all">{t("allCategories") || "All Domains"}</option>
            <option value="statistical">{t("catStatistical") || "Statistical"}</option>
            <option value="technical">{t("catTechnical") || "Technical"}</option>
            <option value="governance">{t("catGovernance") || "Digital Governance"}</option>
            <option value="behavioral">{t("catBehavioral") || "Behavioural"}</option>
          </select>
          ...
          <select
            ...
            className="px-3 py-2 text-xs rounded-xl border border-slate-300 bg-slate-50 text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
          >
            <option value="all">{t("allProficiencyLevels") || "All Levels"}</option>
            <option value="beginner">{t("beginnerLevel") || "Beginner"}</option>
            <option value="intermediate">{t("intermediateLevel") || "Intermediate"}</option>
            <option value="advanced">{t("advancedLevel") || "Advanced"}</option>
          </select>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`bg-white rounded-2xl border transition-all p-5 flex flex-col justify-between shadow-sm hover:shadow-md ${
                isHighMatch ? "border-emerald-300 ring-1 ring-emerald-100" : "border-slate-200"
              }`}
            >
              <div>
                {/* Top Badge Row */}
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {course.competencies.map((c, i) => (
                    <span key={i} className="text-[10px] font-medium bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md border border-blue-100">
                      {c}
                    </span>
                  ))}
                </div>
                ...
                {isEnrolled && (
                  <div className="mt-4 pt-3 border-t border-slate-100 space-y-1.5">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-slate-500">{t("kpiLearningProgress") || "Course Progress"}</span>
