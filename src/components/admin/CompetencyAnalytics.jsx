  ...
  return <div className="space-y-6 max-w-7xl mx-auto">
          ...
          <table className="w-full text-center text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-700 font-bold border-y border-slate-200 uppercase text-[10px]">
                <th className="py-3 px-4 text-left">Competency Area</th>
            ...
            <tbody className="divide-y divide-slate-100 font-mono">
              {heatmapData.map((row, idx) => <tr key={idx}>
    ...
    /* Legend */
        ...
        <div className="flex flex-wrap items-center justify-end gap-4 pt-3 border-t border-slate-100 text-xs text-slate-600">
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-600" /> Expert / High (&ge;80%)</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-blue-600" /> Advanced (65-79%)</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-400" /> Intermediate (50-64%)</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-rose-500" /> Critical Deficit (&lt;50%)</span>
        </div>
      </div>
    </div>;
};
